...
class PreprocessYOLO(object):
    ...
    def process(self, input_image_path):
        ...
        """
        #image_resized = self._load_and_resize(input_image_path)
        image_resized = self._resize(input_image_path)
    ...
    def _load_and_resize(self, input_image_path):
        ...
        image_raw = Image.open(input_image_path).convert('RGB')
        #image_raw = make_square(image_raw, min_size=min(new_resolution[0], new_resolution[1]))
        image_raw = cv2.resize(np.array(image_raw, dtype=np.uint8), new_resolution, interpolation=cv2.INTER_LINEAR)
    ...
    def _resize(self, image):
        ...
        new_resolution = (self.yolo_input_resolution[0], self.yolo_input_resolution[1])
        image_raw = cv2.resize(image_raw, new_resolution, interpolation=cv2.INTER_LINEAR)
    ...
    def _shuffle_and_normalize(self, image):
        ...
        image = np.transpose(np.array(image, dtype=float) / 255,(2,0,1))
        # CHW to NCHW format
