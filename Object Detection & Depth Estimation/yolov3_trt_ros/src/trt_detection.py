...
CFG = "/home/nvidia/xycar_ws/yolov3_onnx_rt/yolov3-tiny_tstl_416.cfg"
TRT = '/home/nvidia/xycar_ws/yolov3_onnx_rt/model_epoch2700.trt'
NUM_CLASS = 6
INPUT_IMG = '/home/nvidia/xycar_ws/src/yolov3_trt_ros/src/video1_2.png'

xycar_image = np.empty(shape=[0])
mapxmapy_exist = False
mapx = np.empty(shape=[0])
mapy = np.empty(shape=[0])
...
class yolov3_trt(object):
    ...
    def _write_message(self, detection_results, boxes, scores, classes):
        ...
        # flatten_xycar_image = (xycar_image.astype(np.uint8)).flatten()
        # detection_results.height = 480
        # detection_results.width = 640
        # detection_results.image = flatten_xycar_image
...
def img_callback(data):
    global xycar_image
    global mapxmapy_exist
    global mapx, mapy
    img = np.frombuffer(data.data, dtype=np.uint8).reshape(data.height, data.width, -1)
    color_xycar_img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    # xycar_image shape (480, 640, 3)
    # Calibrate xycar_image
    mtx = np.array([
    ...
    #tf_image = cv2.undistort(color_xycar_img, mtx, dist, None, cal_mtx)
    if mapxmapy_exist == False:
        mapx, mapy = cv2.initUndistortRectifyMap(mtx, dist, None, cal_mtx, (640, 480), 5)
        mapxmapy_exist = True
    remap = cv2.remap(color_xycar_img, mapx, mapy, cv2.INTER_LINEAR)
    x_, y_, w_, h_ = cal_roi
    # tf_image = tf_image[y_:y_+h_, x_:x_+w_]
    remap = remap[y_:y_+h_, x_:x_+w_]
    xycar_image = cv2.resize(remap, (640, 480))
    xycar_msg = data
